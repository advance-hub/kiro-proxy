...
export default function WarpPanel() {
  ...
  const [addMode, setAddMode] = useState<"firebase" | "apikey">("apikey");
  const [addApiKey, setAddApiKey] = useState("");
  const [showApiKeyBatch, setShowApiKeyBatch] = useState(false);
  const [apiKeyBatchText, setApiKeyBatchText] = useState("");
  const [apiKeyBatchImporting, setApiKeyBatchImporting] = useState(false);
  ...
  const handleAdd = async () => {
    if (addMode === "apikey") {
      if (!addApiKey.trim()) {
        Toast.warning({ content: "请输入 API Key (wk-xxx)" });
        return;
      }
      setAdding(true);
      try {
        const resp = await fetch(`${baseUrl()}/api/warp/credentials`, {
          ...
          body: JSON.stringify({ name: addName.trim() || undefined, apiKey: addApiKey.trim(), refreshToken: addToken.trim() || undefined }),
        });
        const data = await resp.json();
  ...
  const handleApiKeyBatchImport = async () => {
    if (!apiKeyBatchText.trim()) return;
    setApiKeyBatchImporting(true);
    try {
      const resp = await fetch(`${baseUrl()}/api/warp/credentials/batch-import-apikey`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": proxyApiKey },
        body: JSON.stringify({ text: apiKeyBatchText.trim() }),
      });
      const data = await resp.json();
      if (!resp.ok) throw new Error(data.error || `HTTP ${resp.status}`);
      Toast.success({ content: `导入完成: 成功 ${data.data?.success || 0}, 失败 ${data.data?.failed || 0}` });
      setApiKeyBatchText("");
      setShowApiKeyBatch(false);
      fetchCredentials();
      fetchStats();
    } catch (e) {
      Toast.error({ content: `导入失败: ${e}` });
    } finally {
      setApiKeyBatchImporting(false);
    }
  };
  ...
  const columns = [
    ...
    { title: "邮箱", dataIndex: "email", width: 180 },
    {
      ...
      render: (mode: string) =>
        mode === "apikey" ? (
          <Tag color="purple" size="small" type="light">ApiKey</Tag>
        ) : (
          <Tag color="blue" size="small" type="light">Firebase</Tag>
        ),
    },
    {
      title: "状态",
    ...
    { title: "使用", dataIndex: "useCount", width: 60 },
    {
      title: "错误",
    ...
    <div style={{ padding: "20px 24px 32px" }}>
      ...
      <Card bodyStyle={{ padding: "14px 20px" }} style={{ marginBottom: 12, borderRadius: 10 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[
            { method: "POST", path: "/warp/v1/messages", desc: "Anthropic 格式" },
            { method: "POST", path: "/warp/v1/chat/completions", desc: "OpenAI 格式" },
            { method: "GET", path: "/warp/v1/models", desc: "模型列表" },
          ].map((ep) => (
      ...
      <Card bodyStyle={{ padding: "14px 20px" }} style={{ marginBottom: 12, borderRadius: 10 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <Space>
            <div style={{ display: "flex", gap: 4, background: "var(--semi-color-fill-0)", borderRadius: 6, padding: 2 }}>
              <div onClick={() => setAddMode("apikey")} style={{ padding: "4px 10px", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: addMode === "apikey" ? 600 : 400, background: addMode === "apikey" ? "var(--semi-color-primary-light-default)" : "transparent", color: addMode === "apikey" ? "var(--semi-color-primary)" : undefined }}>API Key</div>
              <div onClick={() => setAddMode("firebase")} style={{ padding: "4px 10px", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: addMode === "firebase" ? 600 : 400, background: addMode === "firebase" ? "var(--semi-color-primary-light-default)" : "transparent", color: addMode === "firebase" ? "var(--semi-color-primary)" : undefined }}>Firebase</div>
            </div>
          </Space>
        </div>
        ...
        {addMode === "apikey" ? (
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <Input size="small" placeholder="邮箱/名称 (可选)" value={addName} onChange={setAddName} style={{ width: 160 }} />
              <Input size="small" placeholder="API Key (wk-xxx)" value={addApiKey} onChange={setAddApiKey} style={{ flex: 1 }} />
              <Button size="small" theme="solid" type="primary" icon={<IconPlus />} loading={adding} onClick={handleAdd}>添加</Button>
            </div>
            <Input size="small" placeholder="Refresh Token (AMf-xxx, 可选)" value={addToken} onChange={setAddToken} style={{ marginBottom: 8 }} />
            <div style={{ display: "flex", gap: 8 }}>
              <Button size="small" theme="borderless" icon={<IconUpload />} onClick={() => setShowApiKeyBatch(!showApiKeyBatch)}>
                批量导入上号器格式
              </Button>
            </div>
            {showApiKeyBatch && (
              <div style={{ marginTop: 8 }}>
                <TextArea
                  placeholder={'每行一个，格式: 邮箱----wk-xxx----AMf-xxx\n例如:\nbrandon@live.com----wk-1.474b50dd...----AMf-vBxNCqf...'}
                  value={apiKeyBatchText}
                  onChange={setApiKeyBatchText}
                  rows={5}
                  style={{ fontSize: 12, fontFamily: "monospace" }}
                />
                <Button size="small" theme="light" type="primary" loading={apiKeyBatchImporting} onClick={handleApiKeyBatchImport} style={{ marginTop: 8 }}>导入</Button>
              </div>
            )}
          </>
        ) : (
