...
export default function ClaudeCodePanel() {
  ...
  const loadModels = useCallback(async () => {
    try {
      ...
      const resp = await fetch(`http://${h}:${p}/v1/models`, { headers: { "x-api-key": k }, signal: ctrl.signal });
      clearTimeout(timer);
      if (!resp.ok) return;
  ...
  const handleSyncConfig = async () => {
    ...
    try {
      ...
      try {
        ...
        resp = await fetch(`${baseUrl}/v1/models`, { headers: { "x-api-key": currentApiKey }, signal: ctrl.signal });
        clearTimeout(timer);
      } catch (_) { Toast.warning({ content: `无法连接代理 ${baseUrl}，请先启动代理` }); setSyncing(false); return; }
